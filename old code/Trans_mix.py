...
class PatchTst(nn.Module):
    ...
    def __init__(self, n_hidden_enc=256, n_hidden_dec=256, n_heads=4, dropout=0.1, forecasting_len=96, d_model=1024,
                 encoder_len=96, patch_size=16, enc_layers=2, channels=13):
        assert forecasting_len % forecasting_len == 0

        super().__init__()
        self.forecasting_len = forecasting_len
        self.train = True
        self.encoder_len = encoder_len
        self.encoder_input_layer = nn.Linear(channels * patch_size, d_model)
...
def calc(train_input, test_input, epochs=100, opt="adam", lr=0.2, seq_len=96, pred_len=96, enc_in=13, n_hidden_enc=256,
    ...
    model = PatchTst(forecasting_len=pred_len, encoder_len=seq_len, n_heads=n_heads, n_hidden_enc=n_hidden_enc,
