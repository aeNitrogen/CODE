...
def optimize(model, optimizer, data, batch_size, data_dim, out_dim, criterion, pred_len, seq_len):
    ...
    token, x, y = utils.input_provider.get_input(data, pred_len, seq_len, data_dim, out_dim, two_inputs=True,
                                                 lstm=True)
    ...
    def split_prediction_single(batch_size):
        ...
        for i in range((x.size(0) // batch_size) + batch_add):
            ...
            def closure():
                ...
                out = model.forward(token[start: end, :, :], x[start: end, :, :])
...
def predict(model, data, train_data, batch_size, data_dim, out_dim, criterion, mae_crit, pred_len, seq_len,
    ...
    token, x, y = utils.input_provider.get_input(train_data, pred_len, seq_len, data_dim, out_dim, two_inputs=True,
                                                 lstm=True)
    out = utils.prediction.split_prediction(model, token, batch_size, x_dec=x, lstm=True)
    ret_dict = utils.dict_worker.pred_dict(out, y, criterion, mae_crit, train=True)

    token, x, y = utils.input_provider.get_input(data, pred_len, seq_len, data_dim, out_dim, two_inputs=True, lstm=True)
    ...
    if final:
        ...
        token, x, _ = utils.input_provider.get_input(data, pred_len, seq_len, data_dim, out_dim, two_inputs=True,
                                                     lstm=True, prediction=True)
        prediction = utils.prediction.split_prediction(model, token, batch_size, x_dec=x, lstm=True)
